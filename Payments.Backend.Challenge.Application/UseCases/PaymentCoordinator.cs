    ...
    ILogger<PaymentCoordinator> logger) : IPaymentCoordinator
{
    public async Task<OperationResultDto> ExecuteAsync(PaymentRequestDto request)
    {
        ...
        if (walletPayer == null || walletPayee == null) // it shouldn’t be possible for a user to exist without a wallet, but I’ll check this, though
        {
        ...
        try
        {
            paymentService.ExecutePayment(userPayer, walletPayer, walletPayee, request.Value); // domain concern execute the payment
        ...
        catch (Exception ex)
        {
            ...
            return OperationResultDto.Fail("Fail to execute the payment.");
        }
    }
}